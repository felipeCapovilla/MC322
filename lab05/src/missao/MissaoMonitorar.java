...
public class MissaoMonitorar implements Missao{
    ...
    private ArrayList<Entidade> entidadesObservadas = new ArrayList<>();
    ...
    public void executar(Robo robo, Ambiente ambiente) throws NoRobotException, NullPointerException, SensorMissingException, NoModuleException{
        ...
        if(!isAtivo){
            entidadesObservadas.clear();
            boolean running;
            isAtivo = true;

            do { 
                System.out.println("Entre com as coordenadas do ponto final");
                System.out.print("X: ");
                finalX = ScannerGlobal.scannerNumber();
                
                System.out.print("Y: ");
                finalY = ScannerGlobal.scannerNumber();

                if(robo instanceof RoboAereo){
                    System.out.print("Z: ");
                    finalZ = ScannerGlobal.scannerNumber();
                ...
                if(running){
                    System.out.println("Ponto indisponivel");
                }
        ...
        monitorarEntidade(ambiente, robo);
        ...
        if(
            ...
            OutputLog.addToLogln("Entidades monitoradas:");
            if(entidadesObservadas.isEmpty()){
                OutputLog.addToLogln("\tNenhuma entidade");
            } else {
                for(Entidade ent : entidadesObservadas){
                    OutputLog.addToLogln(String.format("\t-> %s", ent));
                }
            }
    ...
    /**
     * Adiciona as entidades vistas atualmente pelo robo
     */
    private void monitorarEntidade(Ambiente ambiente, Robo robo){
        //Adicionar sem duplicatas
        Set<Entidade> conjunto = new HashSet<>();
        conjunto.addAll(entidadesObservadas);
        conjunto.addAll(robo.get_gerenciadorSensores().getSensorEspacial().detectarEntidades(ambiente, robo));

        //Junta na lista denovo
        entidadesObservadas = new ArrayList<>(conjunto);        
    }
